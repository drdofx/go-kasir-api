...
func (s *ReturnService) ProcessReturnForOrg(orgID, branchID, transactionID int, reason string, items []ReturnItemRequest) (*Return, error) {
	if len(items) == 0 {
		return nil, ErrReturnEmpty
	}
	for _, item := range items {
	...
	exists, err := s.transactionRepo.ExistsByIDForOrg(orgID, transactionID)
	if err != nil {
		return nil, fmt.Errorf("check transaction: %w", err)
	}
	if !exists {
	...
	defer tx.Rollback()
	products, err := s.repo.FindTransactionDetailsForOrg(tx, orgID, transactionID)
	if err != nil {
		return nil, fmt.Errorf("find transaction details: %w", err)
	}
	productMap := make(map[int]LockedProduct)
	...
	returnID, err := s.repo.InsertReturnForOrg(tx, orgID, transactionID, totalRefund, reason)
	if err != nil {
		return nil, fmt.Errorf("insert return: %w", err)
	}
	if err := s.repo.InsertReturnItems(tx, returnID, items, products); err != nil {
	...
	return s.repo.FindByIDForOrg(orgID, returnID)
}
...
func (s *ReturnService) FindAllForOrg(orgID int) ([]Return, error) {
...
func (s *ReturnService) FindByIDForOrg(orgID, id int) (*Return, error) {
